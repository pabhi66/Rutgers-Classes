...
state_struct(Name_list, String_list, state(Name_list, Transform, Accept)) :- 
		member(state(Name_list, Transform, Accept), String_list).
...
next_state_name(state(_, Transition, _), [First_name|Rest_name], Name_list, Rest_name) :- 
	member(transition(First_name, Name_list), Transition).
...
accepts(Initial, String_list, Characters):- 
	next_state_name(Initial, Characters, Name_list, Rest), state_struct(Name_list,String_list,New), accepts(New, String_list, Rest).
