...
;;;SAME AS THE FUNCTION ABOVE, JUST SWITCH ROWS AND COLS
(define (repeat-rows nrepeat pattern)
  (make-pattern (pattern-numcols pattern)
                (* nrepeat (pattern-numrows pattern))
                (lambda (row col) ((pattern-fn pattern) (modulo row (pattern-numrows pattern)) col ))) ;; fill in  here
  )
...
(define (append-cols pattern1 pattern2)
  (define rlength (min (pattern-numrows pattern1) (pattern-numrows pattern2)))
  (define clength (+ (pattern-numcols pattern1) (pattern-numcols pattern2)))
  (make-pattern rlength
                clength
                (lambda (row col)
                (if (< col (pattern-numcols pattern1))
                    ((pattern-fn pattern1) row col)
                    ((pattern-fn pattern2) row (- col (pattern-numcols pattern1))) ;; replace this line
  ))))
...
(define (append-rows pattern1 pattern2)
  (define rlength (+ (pattern-numrows pattern1) (pattern-numrows pattern2)))
  (define clength (min (pattern-numcols pattern1) (pattern-numcols pattern2)))
  (make-pattern rlength
                clength
                (lambda (row col)
                (if (< row (pattern-numrows pattern1))
                    ((pattern-fn pattern1) row col)
                    ((pattern-fn pattern2) (- row (pattern-numrows pattern1)) col) ;; replace this line
  )))
  )
...
(define (flip-cols pattern)
  (make-pattern (pattern-numrows pattern) (pattern-numcols pattern)
                (lambda (row col) ((pattern-fn pattern) row (- (pattern-numcols pattern) (+ col 1))))) ;;replace this line
  )

;;; flip-rows returns a pattern that is the up-down mirror image of pattern
(define (flip-rows pattern)
  (make-pattern (pattern-numrows pattern) (pattern-numcols pattern)
                (lambda (row col) ((pattern-fn pattern) (- (pattern-numrows pattern) (+ row 1)) col ))) ;; replace this line
  )