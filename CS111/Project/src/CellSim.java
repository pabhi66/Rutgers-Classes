...
public class CellSim{
	...
	public static void main(String[] args) {
		...
		//create a CellSimGui object
		CellSimGUI cell = new CellSimGUI(size,5);
		//set colors using nested loop
		for(int i=0; i<tissue.length; i++){
			for(int j=0; j<tissue[i].length; j++){
				if(tissue[i][j] == 'X'){
					cell.setCell(i,j,Color.BLUE);
				}
				if(tissue[i][j] == 'O'){
					cell.setCell(i,j,Color.green);
				}
				if(tissue[i][j] == ' ' || tissue[i][j] == 0){
					cell.setCell(i,j,Color.white);
				}
			}
		}
